    ...
    <div className="min-h-screen bg-[#0B0F19] text-white flex flex-col items-center justify-between p-4 md:p-8">
      ...
      <div className="w-full max-w-5xl bg-[#161D2E] border border-gray-800 p-4 rounded-2xl shadow-xl flex justify-between items-center mb-6 sticky top-4 z-50">
        ...
        <div className="flex items-center gap-4 text-sm font-medium">
          <button onClick={() => router.push('/')} className="text-purple-400 font-semibold cursor-pointer">🏠 Inicio</button>
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-12 items-center my-auto">
        ...
        {/* Columna Izquierda: Presentación y Nueva Imagen Profesional */}
        ...
        <div className="bg-[#161D2E] border border-gray-800 rounded-3xl p-8 shadow-2xl w-full max-w-md mx-auto">
          {activeUser ? (
            <div className="text-center space-y-6 py-6">
              <div className="w-16 h-16 bg-purple-600/20 border border-purple-500 rounded-full flex items-center justify-center mx-auto text-2xl">
                👋
              </div>
              <div>
                <h2 className="text-xl font-bold text-white mb-1">¡Bienvenido de nuevo!</h2>
                <p className="text-xs text-purple-400 break-all">{activeUser}</p>
              </div>
              <p className="text-gray-300 text-sm">
                Ya tienes una sesión activa en ClipStream AI. Puedes ingresar directamente a tu estudio de creación sin volver a loguearte.
              </p>
              <div className="space-y-3 pt-2">
                <button 
                  onClick={() => router.push('/dashboard/create')} 
                  className="w-full py-3.5 bg-purple-600 hover:bg-purple-700 text-white font-bold rounded-xl text-sm cursor-pointer shadow-lg transition"
                >
                  🚀 Ir al Estudio de Creación
                </button>
                <button 
                  onClick={handleLogout} 
                  className="w-full py-2.5 bg-gray-800 hover:bg-gray-700 text-gray-400 hover:text-white font-medium rounded-xl text-xs cursor-pointer transition"
                >
